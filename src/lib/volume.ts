...
export function excludedNote(v: VolumeAssessment): string {
  ...
  if (!v.breakdown.tablesExcluded) {
    ...
    return `${head} Разобрать таблицы в этом файле не удалось — они остались в объёме; для точного подсчёта загрузите работу в .docx.`.trim();
  }

  // У PDF таблица не размечена — она распознаётся по нарисованным ячейкам и
  // раскладке текста на колонки. Предупреждаем прямо: это распознавание, а не
  // чтение разметки, и у нестандартно оформленной таблицы может не сработать.
  const layoutNote = v.breakdown.tablesFromLayout
    ? ' В PDF таблицы распознаются по расположению на странице — при нестандартном оформлении сверьте цифру вручную или загрузите .docx.'
    : '';

  return excluded.length
    ? `Из объёма исключены ${excluded.join(' и ')} — по п. 1.21 они выносятся в приложение.${layoutNote}`
    : `Таблиц и иллюстраций в тексте работы не найдено — вычитать из объёма нечего.${layoutNote}`;
}
